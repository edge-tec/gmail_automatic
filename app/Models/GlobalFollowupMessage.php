...
class GlobalFollowupMessage {
    ...
    public function __isset(string $name): bool {
        return in_array($name, ['body_html', 'is_active']) || isset($this->$name);
    }

    public static function find(int $id): ?self {
        $row = Database::first("SELECT * FROM global_followup_messages WHERE id = :id LIMIT 1", ['id' => $id]);
        return $row ? self::fromRow($row) : null;
    }

    public static function getActiveVariations(int $userId, int $stepNumber): array {
        $rows = Database::query(
            "SELECT * FROM global_followup_messages 
    ...
    public static function getVariationsForStep(int $userId, int $stepNumber): array {
        $rows = Database::query(
            "SELECT * FROM global_followup_messages 
    ...
    public static function getForStep(int $userId, int $stepNumber): array {
        return self::getVariationsForStep($userId, $stepNumber);
    }
    ...
    public static function deleteStep(int $userId, int $stepNumber): bool {
        return Database::execute(
            "DELETE FROM global_followup_messages WHERE user_id = :uid AND step_number = :step",
            ['uid' => $userId, 'step' => $stepNumber]
        );
    }
    ...
    public function save(): bool {
        ...
        if (isset($this->id) && $this->id > 0) {
            $sql = "UPDATE global_followup_messages 
                    SET step_number = :step, variation_name = :name, message = :msg, status = :status, updated_at = {$now} 
                    WHERE id = :id";
        ...
        $sql = "INSERT INTO global_followup_messages 
                (user_id, step_number, variation_name, message, status, created_at)
                VALUES 
                (:uid, :step, :name, :msg, :status, {$now})";
    ...
    public function delete(): bool {
        if (!empty($this->id)) {
            return Database::execute("DELETE FROM global_followup_messages WHERE id = :id", ['id' => $this->id]);
        }
        return false;
    }
    ...
    public static function fromRow(array $row): self {
        ...
        $m->step_number = (int)($row['step_number'] ?? 1);
        $m->variation_name = $row['variation_name'] ?? 'Variation 1';
